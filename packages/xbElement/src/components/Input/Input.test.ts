...
describe('Input.vue', () => {
  ...
  test('test text password icon change',async ()=>{
    ...
    const eyeIcon = wrapper.findComponent({ name: 'FontAwesomeIcon' })
    expect(eyeIcon.exists()).toBeTruthy()
    expect(iconName(eyeIcon.props('icon'))).toBe('eye-slash')
    //点击图标会切换图标，并且值会切换Input类型
    await eyeIcon.trigger('click')
    expect(input.element.type).toBe('text')
    expect(iconName(wrapper.findComponent({ name: 'FontAwesomeIcon' }).props('icon'))).toBe('eye')
  })
})
