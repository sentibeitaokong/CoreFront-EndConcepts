...
class MyPromise {
  ...
  then(onFulfilled, onRejected) {
    onFulfilled =
      typeof onFulfilled === 'function' ? onFulfilled : value => value //onFulfilled是方法说明还是promise对象，否则是常量
    onRejected =
  ...
  static all(values) {
    return new MyPromise((resolve, reject) => {
      ...
      if (!isIterable(values)) {
        throw new TypeError(
          `${values} is not iterable (cannot read property Symbol(Symbol.iterator))`,
        )
      }
      let len = values.length
      if (len === 0) {
