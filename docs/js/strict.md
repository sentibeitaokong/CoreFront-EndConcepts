...
*  **消除 JavaScript 语言中一些不合理、不严谨的特性**：减少代码的错误率。
*  **纠正 JavaScript 引擎的一些不安全操作**：例如，防止意外创建全局变量。
*  **提高编译器效率**：允许 JS 引擎进行更多的优化，提升代码运行速度。
*  **为未来 JavaScript 版本做好铺垫**：禁止一些未来可能成为关键字的标识符。
