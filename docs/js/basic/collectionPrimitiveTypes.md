...
- **特性**：
  - **动态大小**：不需要预先指定长度，随时可以增删。
  - **混合类型**：同一个数组可以存储不同类型的数据（数字、字符串、对象、函数等）。
  - **零基索引**：索引从 `0` 开始。
...
- **常规迭代方法**:
  ...
  _注意：`map`, `filter` 等返回新数组的方法会返回相同类型的 TypedArray。_
...
- **Q3: 如何将 `Map` 序列化为 javascriptON？**
  ...
  // 转换为对象
  const javascriptonStrFromObject = javascriptON.stringify(
