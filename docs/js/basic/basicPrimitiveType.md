...
_(注：`Symbol` and `BigInt` 也有对应的包装对象机制，但不能通过 `new` 显式创建)_
...
- **Date**: 处理时间，注意月份 0 起始。
- **RegExp**: 强大的文本处理工具，核心是 `exec` 和 `test`。
- **原始值包装类型 (String/Number/Boolean)**:
