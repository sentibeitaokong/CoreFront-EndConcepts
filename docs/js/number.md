...
*   **Q1: 为什么 `0.1 + 0.2 !== 0.3`？**
    *   这是所有使用 IEEE 754 双精度浮点数语言的通病。0.1 和 0.2 在二进制中是无限循环小数，相加后精度丢失，结果是 `0.30000000000000004`。
    *   **解决**:
        1.  使用 `Number.EPSILON` (ES6 引入的极小常量) 进行误差范围比较：`Math.abs(0.1 + 0.2 - 0.3) < Number.EPSILON`。
        2.  转成整数计算（例如金额计算单位用“分”而不是“元”）。
        3.  使用专门的库（如 `decimal.js`）。

*   **Q2: 全局 `isNaN` 和 `Number.isNaN` 有什么区别？（高频面试题）**

    *   **`isNaN(value)`**: 会先尝试将 `value` **转换**为数字。如果转换结果是 `NaN`，就返回 `true`。因此 `isNaN("foo")` 为 `true`（"foo" 转数字是 NaN）。
    *   **`Number.isNaN(value)`**: **不转换**。先判断类型是否为 Number，再判断值是否为 `NaN`。因此 `Number.isNaN("foo")` 为 `false`。**这是判断 NaN 的正确方法**。

*   **Q3: `parseInt` 和 `Math.trunc` 去除小数有什么区别？**

    *   `Math.trunc` 是纯数学运算，直接舍去小数位。
    *   `parseInt` 会先将参数转换为**字符串**，然后解析。这可能导致意外：
        ```js
        Math.trunc(0.0000001); // 0
        parseInt(0.0000001);   // 1  (因为 0.0000001 转字符串是 "1e-7"，parseInt 解析到了 '1')
        ```

*   **Q4: 什么时候应该用 `BigInt`？**
    *   仅当你需要处理**超过 2^53 - 1** 的整数（如高精度时间戳、加密算法、特定 ID）时。不要用它代替普通的 `Number`，因为 `BigInt` 的性能通常较差且操作受限。

*   **Q5: `Number.EPSILON` 是什么？**
    *   它是 JavaScript 中能表示的 1 与大于 1 的最小浮点数之间的差值（大约是 `2.22e-16`）。它实际上定义了浮点数计算的**最小误差范围**。如果两个数的差值小于它，我们可以认为这两个数相等。
