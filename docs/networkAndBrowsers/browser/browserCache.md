...
- **Service Worker：** 运行在浏览器背后的独立线程，常用于实现离线缓存（PWA技术）。它可以自由控制缓存哪些文件、如何匹配缓存等，灵活性最高。
...
### 5.2 `from memory cache` 和 `from disk cache` 的区别是什么？
...
### 5.3 如何配置才能让前端发版（更新代码）后，用户立刻能看到最新效果，而不用手动清缓存？
...
### 5.4 为什么有了 ETag 还要保留 Last-Modified？
...
### 5.5 `Cache-Control: no-cache` 和 `Cache-Control: no-store` 的区别是什么？
