...
function Child({ onMsgSend }) {
  return (
    <button onClick={() => onMsgSend('爸爸，我饿了！')}>向父亲发送消息</button>
  )
}
```
...
function PageLayout({ header, sidebar, mainContent }) {
  return (
    <div className="layout-container">
      {/* 坑位 1 */}
      <header className="top-nav">{header}</header>
...
![Logo](/img/reactPropsDrilling.webp)
