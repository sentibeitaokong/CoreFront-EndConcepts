...
- **答**：**不再强制需要了。**
  ...
  - **现在 (React 17 的新 JSX 转换机制)**：现代编译器（如新版 Babel、Vite/esbuild、SWC）在底层实现了全新的转换逻辑。它们会在编译时**自动且悄悄地**帮你引入 `jsx-runtime` 来处理 JSX，彻底把你从每个文件都要写 `import React` 的体力劳动中解放了出来。_(除非你需要显式使用 `React.useState` 等具体的 API，但通常我们也是使用 `import { useState }` 解构引入的)_。
