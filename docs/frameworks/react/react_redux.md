...
_这是现代业务开发中 95% 以上时间你会用到的 API，它极大地减少了模板代码（Boilerplate）。_
...
<button onClick={() => dispatch(fetchUserById(9527))}>拉取用户信息</button>
```

## 5. React-Redux 核心 API (绑定 React)

_Redux 本身和 React 没有任何关系。`react-redux` 库负责把这俩强行缝合在一起。_
...
_虽然有了 RTK，你很少直接写它们，但在阅读源码或老项目时必须认识。_
