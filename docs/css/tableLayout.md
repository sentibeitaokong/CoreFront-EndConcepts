...
_注意：如果表格的父容器设置了 `overflow: hidden`，`sticky` 可能会失效。_

### 3.5 `visibility: collapse` 在表格里的特殊表现？
