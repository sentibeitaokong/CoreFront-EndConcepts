...
*   **能用 Flex/Grid 就别用 Float/Inline-Block**：现代布局系统规避了几乎所有传统文档流的坑（如清除浮动、空白间隙）。
*   **理解盒模型**：始终使用 `box-sizing: border-box`。
*   **警惕 Margin**：当父子元素边缘重合时，优先想到 BFC 或使用 Padding 代替 Margin。
*   **图片处理**：习惯性把布局用的图片设为 `display: block`。
