...
- **`localStorage`**: **永久存储**。除非用户手动清理或代码删除，否则数据永远存在。
- **`sessionStorage`**: **会话存储**。浏览器窗口（Tab）关闭后，数据自动销毁。
...
- _注意：在当前修改数据的页面，不会触发自己的 storage 事件。_
...
- _特例_：如果你在 Tab A 通过 `window.open` 或右键“在新标签页打开”链接，Tab B 会**复制**一份 Tab A 的 sessionStorage（之后两者独立）。
...
- _注：极少数旧版浏览器（如老 Safari）在隐身模式下调用 `setItem` 会直接报错，建议 try-catch。_
