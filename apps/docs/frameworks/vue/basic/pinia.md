...
_现在，在任意的 Options Store 中你就可以直接 `this.router.push` 了。(注：在 Setup Store 中无法通过 `this` 访问，通常直接 `import router` 更符合现代 ESM 规范)。_
...
export function persistPlugin({ store }) {
  ...
  store.$subscribe(
    ...
    { detached: true },
  ) // detached: true 保证即使注册插件的组件被销毁了，监听器依然在后台全天候运行！
}
```
