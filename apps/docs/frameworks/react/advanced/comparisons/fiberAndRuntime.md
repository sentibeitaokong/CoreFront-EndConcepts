...
- React 选择将所有信息集中在一个节点上，单一 Fiber 节点 = 状态容器 + 更新队列 + 副作用记录 + 调度信息 + 树拓扑，通过 alternate 实现双缓冲
- Vue 3 则保持了响应式系统的简洁性，将优化集中在编译时与依赖追踪上，通过组件实例存状态、VNode 存 UI 描述、Effect 管理依赖追踪、Scheduler 管理更新时序，直接完成新旧 VNode 比较。
