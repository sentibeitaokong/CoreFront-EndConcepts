...
Vue 3 的运行时也内置了一个调度器（`runtime-core/src/scheduler.ts`），但它与 React 的 `scheduler` 在设计理念和实现复杂度上有本质区别。下面从多个维度进行对比。

### 11.1 设计目标差异

| 维度           | React Scheduler                              | Vue 3 Scheduler                            |
| -------------- | -------------------------------------------- | ------------------------------------------ |
| **核心目标**   | 实现**可中断渲染**（Concurrent Mode）        | 实现**异步批量更新**（Batching）           |
| **解决的问题** | 大型组件树 Diff 长时间占用主线程导致掉帧     | 同一轮事件循环中多次数据变更合并为一次更新 |
| **复杂度来源** | 需要与浏览器抢时间，需要时间切片和优先级调度 | 不需要时间切片，只需控制异步执行时机       |

React 的调度器是一个"**微型操作系统内核**"级别的调度器——它要管理 CPU 时间片，决定哪个任务在什么时候执行多久；而 Vue 3 的调度器更像一个"**任务编排器**"——按固定顺序（pre → job → post）执行任务即可。

### 11.2 核心架构对比

| 维度           | React Scheduler                                      | Vue 3 Scheduler                                                          |
| -------------- | ---------------------------------------------------- | ------------------------------------------------------------------------ |
| **任务载体**   | `MessageChannel`（宏任务）                           | `Promise.resolve().then()`（微任务）                                     |
| **队列结构**   | 双队列：`taskQueue` + `timeQueue`，小顶堆排序        | 三队列：`pendingPreFlushCbs` + `queue` + `pendingPostFlushCbs`，简单数组 |
| **排序依据**   | `expirationTime` / `startTime`（动态计算）           | 无排序，仅按队列顺序（pre → job → post）执行                             |
| **优先级体系** | 5 级：Immediate / UserBlocking / Normal / Low / Idle | 无优先级，仅有"**执行时机**"区分（pre / sync / post）                    |
| **时间切片**   | ✅ `shouldYieldToHost()`，5ms 切片                   | ❌ 无时间切片，一次性清空所有队列                                        |
| **可中断恢复** | ✅ `continuationCallback` 机制                       | ❌ 任务不可中断，必须执行完                                              |
| **防饿死机制** | ✅ timeout → expirationTime 过期自动提升             | ❌ 无（因为没有优先级，也就不存在饿死问题）                              |
| **延迟调度**   | ✅ `timeQueue` + `setTimeout`，支持 delay            | ❌ 无延迟调度能力                                                        |
| **取消机制**   | ✅ `task.callback = null`，惰性删除                  | ✅ `queue.includes(job)` 去重，天然幂等                                  |
| **独立性**     | 独立 `scheduler` 包，与 React 解耦                   | 内嵌于 `runtime-core`，与 Vue 响应式系统耦合                             |

### 11.3 为什么 Vue 3 不需要时间切片？

这是导致两个调度器差异最大的根本原因，可以从三个层面理解：

- **组件粒度不同。** React 的渲染单元是 Fiber 树——每次 `setState` 可能触发整棵组件树的全量 Diff。即使有 `memo`，不优化的场景下子组件仍会递归遍历。当组件树足够大（上千个节点），单次 Diff 就可能超过一帧（16.6ms）。Vue 3 通过模板编译阶段的静态标记（PatchFlags）和响应式依赖精确追踪，组件更新时只遍历真正变化的部分，单次 `patch` 通常远小于 5ms，天然不需要切片。
- **批处理机制不同。** React 18 之前，`setState` 在异步回调中不会自动批处理（需要 `unstable_batchedUpdates`），Concurrent Mode 的 Scheduler 统一了批处理和优先级调度。Vue 3 的响应式系统默认将所有 `trigger` 收敛到同一个微任务（`queueFlush` → `Promise.then`），天然批处理。
- **异步载体的选择逻辑相反。**

```mermaid
flowchart LR
    subgraph React["React: 选宏任务"]
        R1[需要让出主线程] --> R2[微任务会一口气清空<br/>浏览器无机会插入渲染帧]
        R2 --> R3[选 MessageChannel 宏任务<br/>每个 tick 间隙可绘制]
    end
    subgraph Vue["Vue 3: 选微任务"]
        V1[不需要让出主线程] --> V2[宏任务延迟 ≥ 0ms<br/>累计延迟影响首屏]
        V2 --> V3[选 Promise.then 微任务<br/>尽可能快地在当前帧完成]
    end
```

React 选宏任务是为了**插入渲染帧**（让出给浏览器），Vue 3 选微任务是为了**尽快完成更新**（减少异步开销）。
...
```markdown
scheduleCallback → 推入 taskQueue/timeQueue（小顶堆按优先级排序）
→ MessageChannel.onmessage → performWorkUntilDeadline（记录切片起点）
→ flushWork → workLoop 循环：
├ 取堆顶（最高优先级任务）
├ 检查 shouldYieldToHost（超 5ms → break，重新 postMessage）
├ 执行 task.callback(didTimeout)
└ callback 返回 function → 挂回延续回调，下次继续
→ 队列空 → 停止消息循环
```
...
```markdown
trigger → effect.scheduler → queueJob(instance.update)
→ queueFlush() → Promise.then(flushJobs)
→ flushJobs：
├ 1. flushPreFlushCbs（watch flush:pre）
├ 2. while(queue.length) shift+执行（组件渲染）
├ 3. flushPostFlushCbs（watch flush:post）
└ 4. 若刷新期间有新任务入队 → 递归 flushJobs
→ 全部完成 → currentFlushPromise = null
```

### 11.5 适用场景总结

| 场景                     | React Scheduler                      | Vue 3 Scheduler                  |
| ------------------------ | ------------------------------------ | -------------------------------- |
| 大型应用（数千组件）     | 时间切片保证不卡顿                   | 依赖静态优化，极端场景有风险     |
| 高优先级交互（如输入框） | ImmediatePriority 立即同步执行       | 无法提升优先级，只能等微任务队列 |
| 动画 / 持续交互          | UserBlockingPriority + 250ms timeout | 无优先级区分，可能与渲染竞争     |
| 延迟 / 空闲任务          | LowPriority / IdlePriority + delay   | 不支持，所有任务同批执行         |
| 常规中小型应用           | 功能过剩，但无负面影响               | 简洁够用，运行时开销更低         |

### 11.6 设计哲学对比

```markdown
React: "调度是框架的第一公民"
→ 可中断渲染是 Concurrent Mode 的基石
→ 需要精确控制"何时执行、执行多久、谁能抢占"
→ 因此需要一个完整的自研调度器

Vue 3: "调度是最小必要实现"
→ 响应式精确追踪 + 编译优化已大幅减少 Diff 开销
→ 只需控制"在一个 tick 内批处理，分 pre/job/post 三个阶段"
→ 因此内置一个轻量调度器即可满足需求
```

> [!NOTE]
> **一句话总结**：React Scheduler 解决的是"**如何在有限时间内排优先级、切片执行**"的问题，而 Vue 3 Scheduler 解决的是"**如何在正确的时间按正确顺序执行**"的问题。前者是操作系统级调度问题，后者是异步编排问题。两者并不在一个复杂度层级上，但这种差异恰好反映了两个框架在响应式更新粒度上的根本分工差异——React 把复杂性放在调度层，Vue 3 把复杂性放在编译优化和依赖追踪层。
