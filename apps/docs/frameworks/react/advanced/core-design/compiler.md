...
## 5. React编译器 vs Vue 3 编译器

虽然都在提“**编译期优化**”，但受限于底层哲学，两者的优化方向和技术难度截然不同：

| 维度               | React Compiler (React 19+)                                                             | Vue 3 Compiler (compiler-core)                                                    |
| ------------------ | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| **优化的根本目标** | 维持 `UI = f(state)` 范式，通过自动缓存引用，阻止函数重复执行和虚拟 DOM 树的过度比较。 | 绕过全量虚拟 DOM 比对，将视图更新降级为精准的指令执行。                           |
| **输入的语法约束** | **极其宽松**：处理图灵完备的 JavaScript / JSX，存在极大的静态分析难度。                | **强约束**：处理基于指令的 HTML 模板 DSL（`v-if`, `v-for`），语义明确，易于分析。 |
| **静态节点的处理** | 不做特殊处理，依然会被创建并在运行时判断是否复用。                                     | **静态提升 (Static Hoisting)**：编译期彻底抽离，运行时完全跳过比对。              |
| **动态节点的处理** | 通过 `_c` 数组缓存动态节点的引用以触发运行时的 Bailout。                               | 为动态节点打上 **PatchFlags**（靶向更新），构建 Block Tree，做到真正的按需更新。  |
| **工程实现难度**   | 极高。涉及编译器级别的别名分析、逃逸分析、SSA 构建，开发周期长达数年。                 | 较低。通过正则表达式与 AST 转换即可确定模板节点属性的动静状态。                   |
