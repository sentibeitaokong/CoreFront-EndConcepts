...
| 维度           | React（Lane + Scheduler）                 | Vue 3                                       |
| -------------- | ----------------------------------------- | ------------------------------------------- |
| **优先级模型** | Lane 二进制位掩码                         | `nextTick` 队列 + `flushJobs`               |
| **任务调度**   | 独立 Scheduler 包，时间切片               | Promise.then 微任务队列                     |
| **可中断渲染** | ✅ 长任务可暂停并恢复                     | ❌ 渲染是同步的（但采用细粒度更新降低成本） |
| **并发策略**   | 时间切片 + 优先级抢占                     | 响应式细粒度更新 + 异步队列                 |
| **复杂度**     | 高（Scheduler 是独立子系统）              | 低（依赖 JS 原生微任务）                    |
| **Transition** | ✅ `startTransition` / `useDeferredValue` | —（响应式系统自动处理）                     |

Vue 的设计哲学是**让更新足够细粒度**，使得单次渲染开销极低，因此不需要时间切片。React 则通过**调度控制**在开销较大时保持响应。
