...
_(⚠️ 注意：`useActionState` 是 React 19 引入的全新核心 Hook。在 React 18 的早期实验版本中，它曾被称为 `useFormState`。本文基于 React 19 最新标准编写。)_
...
function ArticleCard({ article }) {
  // 初始状态传入文章当前的点赞信息
  const [likeState, executeLike, isLiking] = useActionState(toggleLikeAction, {
  ...
  return (
    <div className="card">
      ...
      <button onClick={() => executeLike(article.id)} disabled={isLiking}>
