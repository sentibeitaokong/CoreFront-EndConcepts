...
_(例外：`aria-_`和`data-_`属性依然使用短横线写法，如`data-id="1"`。)_
...
这意味着你需要用到**两层大括号** <span v-pre>`{{}}`</span>外层代表进入 JS 环境，内层代表这是一个普通 JS 对象。且所有的 CSS 属性名都必须变成驼峰式！
...
- **答**：这是 `&&` 运算符的一个致命物理陷阱。
  ...
  return <div>{messages.length && <ul>...</ul>}</div>
  ```
