...
- **长驻页面不刷新**。单页应用、大屏、IM 一开一整天，泄漏持续累积：几小时后内存翻倍、掉帧甚至崩溃。
...
- **分配**：新对象先进新生代 From 区（semi-space），超阈值的**大对象**直接进老生代。
- **第一次 Scavenge**：From 区写满触发，存活对象复制到 To 区，From 清空。
- **晋升**：经历两轮 Scavenge 仍存活，或 Scavenge 后 To 区占用超 25% 时被“晋升”到老生代（保证 To 区有余量）。
- **老生代回收**：占用达到动态阈值时触发标记-清除-整理，配合增量 / 并发标记降停顿。
...
`Map` 的迭代顺序等于插入顺序，LRU 只需十几行。若键是对象、希望“**对象没了缓存自动清掉**”，换成 `WeakMap`。
...
### 5.2 Chrome DevTools Memory 面板

- **Heap Snapshot（堆快照）**：在不同时间点拍快照并对比，找出未被回收的对象。
- **Allocation on timeline（时间轴分配）**：录制分配过程，定位反复分配不释放的对象。
- **Performance 面板**：观察 JS Heap 曲线是否「锯齿状」（有回收）还是「持续上升」（泄漏）。
...
### 5.6 Allocation instrumentation on timeline
...
### 5.8 `performance.memory` 与 `process.memoryUsage()`
...
### 8.2 `WeakMap` 为什么能避免泄漏？它和 `Map` 有什么区别？
...
- 因此适合“**给对象挂附属数据**”，不适合需要统计、过期、限容量的业务缓存——那种场景用 `Map` + LRU。
...
### 8.6 DevTools 里怎么确认是“泄漏”而不是“还没回收”？
