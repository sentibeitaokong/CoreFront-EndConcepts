...
2.  **I/O callbacks (待定回调)**: 某些系统操作（如 TCP 错误）的回调。
...
**每个阶段结束后都会清空微任务队列，再进入下一个阶段。**

---

#### **Node.js 中的宏任务 (Macrotasks) 和微任务 (Microtasks)**

##### **微任务 (Microtasks)**

微任务的优先级**高于宏任务**，因此每个阶段结束、进入下一个阶段前都会先清空微任务队列。

- **常见的微任务来源**: **`Promise` 的 `then()`/`catch()`/`finally()`**、**`async/await`**（`await` 后面的代码）、Node.js 特有的 **`process.nextTick()`**（**优先级最高**，在当前操作结束之后、下一个事件循环阶段开始之前立即执行，甚至比 Promise 回调还早），以及 ES 标准的 **`queueMicrotask()`**。
...
- **常见的宏任务来源**: `setTimeout()`、`setInterval()` 的定时器回调，`setImmediate()`（`check` 阶段的回调，Node.js 特有），文件读写、网络请求等 I/O 回调（`fs.readFile`、`http.get` 等），以及 `script` 整体代码。
...
- **两者的先后顺序取决于事件循环当前的状态**：`setTimeout(fn, 0)` 的延时按 `1ms` 处理，若进入 `timers` 阶段时这 1ms 已过去，`timers` 的回调就先执行；否则会先跳过 `timers`，走 `poll` → `check`，让 `setImmediate` 先执行。**但在纯粹的 I/O 回调内部，`setImmediate` 总是优先于 `setTimeout(0)`。**
...
- **`process.nextTick()` 具有最高的优先级**：当前宏任务执行完毕后，所有微任务（`nextTick` 与 `Promise`）被清空，其中 `nextTick` 又优先于 `Promise`。
- **微任务 (Microtasks)** 会在每个事件循环阶段结束时被清空。
- **宏任务 (Macrotasks)** 分散在事件循环的不同阶段，每个阶段只处理该阶段的宏任务。
- `setTimeout(0)` 和 `setImmediate()` 在没有 I/O 操作时的相对顺序并不确定（`setTimeout` 可能先也可能后）；但在 I/O 回调内部，`setImmediate` 一定先于 `setTimeout(0)` 执行。
...
- `async/await` 是 `Promise` 的语法糖，行为完全一致。
