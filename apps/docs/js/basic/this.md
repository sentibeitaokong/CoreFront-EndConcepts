...
| 绑定规则   | 触发条件                  | `this` 指向                            |
| :--------- | :------------------------ | :------------------------------------- |
| 默认绑定   | 独立函数调用              | 全局对象（严格模式 `undefined`）       |
| 隐式绑定   | `obj.foo()` 形式调用      | 调用位置的上下文对象（属性链最后一层） |
...
### 7.1 `call`、`apply`、`bind` 三者的区别？

| 方法                   | 调用时机   | 传参方式       | 返回值                 |
| :--------------------- | :--------- | :------------- | :--------------------- |
| `fn.call(obj, ...)`    | 立即调用   | 参数逐个传入   | 函数返回值             |
| `fn.apply(obj, [...])` | 立即调用   | 参数以数组传入 | 函数返回值             |
| `fn.bind(obj, ...)`    | 返回新函数 | 参数逐个传入   | 绑定了 `this` 的新函数 |

> `apply` 与 `call` 唯一区别是参数形式（数组 vs 逐个）；`bind` 不立即执行，常用于事件回调与柯里化。

### 7.2 箭头函数的 `this` 有什么特点？

箭头函数没有自己的 `this`，会**捕获定义时**所在词法作用域的 `this`，且**无法被修改**（`call`/`apply`/`bind`/`new` 都无效）。

```js
const obj = {
  a: 1,
  foo: () => console.log(this.a), // this 指向定义时外层作用域
}
obj.foo() // undefined（外层是全局，this 为 window）
```

### 7.3 为什么 `setTimeout` 回调里的 `this` 会丢失？

`setTimeout(fn)` 内部以**独立函数**方式调用 `fn`，属于默认绑定，`this` 指向全局对象（严格模式 `undefined`），而非你期望的对象。
...
**解决**：用箭头函数、`bind`，或 `const self = this`。
...
`new` 绑定 > 显式绑定（`call`/`apply`/`bind`）> 隐式绑定（`obj.foo()`）> 默认绑定。

### 7.6 什么是「隐式丢失」？如何解决？

函数以 `obj.foo` 形式被引用后，再以**独立函数**调用（如 `const bar = obj.foo; bar()`，或作为回调传入），会丢失上下文对象，退回默认绑定。

**解决**：用 `bind` 硬绑定，或改用箭头函数。
