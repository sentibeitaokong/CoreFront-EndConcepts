...
无论你在函数的哪一行声明 `var`，JavaScript 引擎在运行代码前，都会把**声明**（注意：只是声明，不是赋值）“移动”到当前作用域的顶部。
...
_(注：如果使用 `let` 或 `const` 这样做会直接抛出 `SyntaxError`)_
...
**解释**：当 `setTimeout` 的回调函数执行时，循环已经结束了，此时全局的 `i` 已经变成了 3。因为所有回调函数引用的都是同一个 `i`。
