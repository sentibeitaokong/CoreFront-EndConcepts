...
在 JavaScript 中，比较两个值是否“相等”比看起来要复杂，因为存在两种主要的比较方式：**严格相等**和**宽松相等**，此外还有 ES6 引入的**同值相等**。理解它们的区别是编写健壮、无歧义代码的关键。

> [!TIP] 相关阅读
> 隐式类型转换的根源在于 [数据类型](/js/basic/eightTypes) 与 [基本引用类型](/js/basic/basicPrimitiveType) 的装箱/拆箱机制。
...
5. **`Object == (String | Number | Symbol)`？** → 将 `Object` 转换为原始类型（会调用内部的 ToPrimitive 操作，根据上下文决定优先调用 `valueOf()` 还是 `toString()`）再比较。

![相等性转换规则](/img/equal.png)
...
function objectIs(x, y) {
  // 情况 1: 处理 +0 和 -0
  // 如果 x 和 y 都是 0，它们通过 === 比较会是 true，但需要区分 +0 和 -0。
  // 技巧：1 / +0 === Infinity，而 1 / -0 === -Infinity。
  // 所以如果它们的倒数不相等，说明一个是 +0，另一个是 -0。
  if (x === 0 && y === 0) {
    return 1 / x === 1 / y
  }
...
> **用途：** `Object.is()` 在需要精确区分 `NaN` 或 `+0`/`-0` 的数学计算或算法场景中非常有用。在日常业务逻辑中，`===` 依然是首选。

## 4. 常见问题与陷阱

以下是开发者（尤其是初学者）最容易踩的坑，它们几乎都与 `==` 的隐式类型转换有关。

### 4.1 `true`、`false` 与数字的比较
...
**✅ 解决方案**：使用 `===` 进行显式检查，避免依赖 `null` 的隐式转换。
...
| **`==`**          | **宽松相等** | 是       | 几乎从不。唯一例外可能是 `val == null`（等价于 `val === null |     | val === undefined`），但显式检查更清晰。 |
