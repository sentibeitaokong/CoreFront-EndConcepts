...
## 11. MicroApp vs Wujie

| 对比维度          | 无界 (Wujie)                                  | micro-app (v1.0+ iframe 模式)                        |
| ----------------- | --------------------------------------------- | ---------------------------------------------------- |
| **DOM 桥接原理**  | 原型重写 + `bind` 绑定到 ShadowRoot           | Element Patch + 容器查询范围限定                     |
| **样式隔离策略**  | 强制依赖 **Shadow DOM** (物理级绝对隔离)      | 默认采用 **Scoped CSS** 属性隔离 (逻辑级隔离)        |
| **UI 库弹窗兼容** | **较差**，必须手动配置挂载节点穿透 ShadowRoot | **较好**，默认无物理边界，全局挂载自然               |
| **路由系统**      | 拦截 History API 进行双向同步                 | 引入庞大的**虚拟路由 (Virtual Router)** 系统管理状态 |
| **设计哲学**      | 激进、纯粹、追求极致的物理隔离                | 兼容、平滑、追求在旧有虚拟沙箱体系上的工程化延伸     |
