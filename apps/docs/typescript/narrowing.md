...
`as` 在编译产物里不生成任何代码，它只影响类型检查。所以 `x as User` 之后，如果 `x` 运行时根本不是 `User`，程序会一路带着错误的类型假设跑下去，直到某个字段访问崩掉——而崩溃点往往离出错点很远，极难排查。详见上面第 7 节。
...
- **`filter` 之后数组类型没变窄**：`filter` 的签名只声明“返回值与原数组元素类型相同”，它**没有承诺**会改变类型，TS 只能按签名办事。三种改法——显式类型谓词 `arr.filter((v): v is string => v !== null)`（最稳，任何版本都支持）、依赖 TS 5.5+ 的自动推断、或改用 `flatMap` 返回单元素数组。完整代码见 8.2。
- **闭包 / 回调里访问变量报“可能为 `null`”**：闭包**不保证在收窄的那个时刻执行**，编译器无法确认回调真正跑起来时，外层变量是否已经被别处改掉，于是撤销之前的所有收窄结论。解法是把值捕获进 `const`（必要时先解构）。详见 8.1 与 8.3。

**通用排查顺序**，按这个走基本都能定位：

1. **变量是不是 `let` 且被重新赋值过？** —— 改 `const`，或先解构到局部常量。
2. **是不是在闭包 / 回调里访问的？** —— 同上，捕获成局部 `const`。
3. **收窄的是不是对象属性而不是变量本身？** —— 中间是否夹了函数调用？解构成局部常量。
4. **是不是数组方法（`filter` / `map`）没保留类型？** —— 补类型谓词。
5. **以上都不是** —— 那可能是控制流分析真的覆盖不到，写一个 `is` 类型守卫，把判断逻辑显式化。
